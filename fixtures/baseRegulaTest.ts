...
>({
  ...
  workerStorageState: [
    async ({ browser }, use) => {
      ...
      const authFile = path.resolve('regula-test-results', `.auth/${parallelIndex}.json`);
      if (fs.existsSync(authFile)) {
        await use(authFile);
        return;
      } else {
        const context = await browser.newContext({ storageState: undefined, baseURL: conf.use.baseURL });
